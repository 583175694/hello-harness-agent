...
export class ReportCreateTool implements AgentTool<CreateReportInput, CreateReportResult> {
  ...
  definition() {
    return {
      name: this.name,
      description:
        '创建一份可独立阅读、可保存和后续复用的正式 Markdown 报告。对用户整体意图判断为一项或多项正式交付物时使用；同一运行可以多次调用，每次创建一份独立报告。不要仅因内容较长而使用，也不要把完整报告正文直接放在最终聊天回复中。必须提供标题、摘要和完整正文。调用成功后，最终回复只需概括结论并提示用户打开报告。可关联网页来源和当前会话材料。',
      parameters: {
        type: 'object',
