import { AGENT_ERROR_CODES, normalizeSourceUrl } from '@harness/agent-protocol';
import type {
  WebFetchBudget,
  WebFetchSkippedItem,
  WebFetchStopReason,
...
import { createToolRunStateKey } from './tool-run-state';

type UrlReservation =
  { status: 'accepted'; requestedUrl: string } | { status: 'skipped'; result: WebFetchSkippedItem };
...
const WEB_RESEARCH_RUN_STATE_KEY = createToolRunStateKey<WebResearchRunState>('web-research');

// 保存单次 Agent run 的网页调查来源、预算和去重状态。
export class WebResearchRunState {
  ...
  constructor(
    ...
    this.allowFetchUrls(this.extractUserUrls(latestUserContent));
  }

  allowFetchUrls(urls: string[]): void {
    for (const url of urls) {
      try {
        this.allowedFetchUrlKeys.add(normalizeSourceUrl(url));
      } catch {
        // 非法 URL 由输入协议或 URL Guard 返回正式错误。
      }
    }
  }
    ...
    const reservations = urls.map((requestedUrl): UrlReservation => {
      ...
      if (this.stopReason) {
        return {
          ...
          result: this.skipped(
            requestedUrl,
            AGENT_ERROR_CODES.fetchBudgetExceeded,
            '本轮联网调查已经停止。',
          ),
        };
      }
      ...
      if (this.acceptedUrlKeys.has(key) || this.documentUrlKeys.has(key)) {
        return {
          ...
          result: this.skipped(
            requestedUrl,
            AGENT_ERROR_CODES.fetchDuplicateSkipped,
            '本轮已读取过等价网页。',
          ),
        };
      }
  ...
  registerDocument(input: {
    ...
    if (
      this.contentHashes.has(input.contentHash) ||
      aliases.some((key) => this.documentUrlKeys.has(key))
    ) {
      return false;
    }
    for (const key of [normalizeSourceUrl(input.requestedUrl), ...aliases]) {
      this.documentUrlKeys.add(key);
    }
  ...
  private extractUserUrls(content: string): string[] {
...
export function getWebResearchRunState(
  context: Pick<ToolExecutionContext, 'latestUserContent' | 'runState'>,
): WebResearchRunState {
  return context.runState.getOrCreate(
    WEB_RESEARCH_RUN_STATE_KEY,
    () =>
      new WebResearchRunState(
        WEB_RESEARCH_POLICY.maxUrlsPerRun,
        WEB_RESEARCH_POLICY.maxExternalPassageCharacters,
        context.latestUserContent,
      ),
  );
}
