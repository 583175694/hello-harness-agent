...
export class WebFetchUrlGuard {
  ...
  async validate(rawUrl: string, redirect = false): Promise<GuardedWebUrl> {
    ...
    if (ipaddr.isValid(hostname)) this.assertPublicAddress(hostname);
      ...
      let addresses;
      try { addresses = await this.resolveDns(hostname); } catch {
      ...
      if (!addresses.length) {
        throw new WebFetchError(AGENT_ERROR_CODES.fetchUpstreamFailed, '网页域名没有可用地址。', true);
      }
  ...
  private assertPublicAddress(value: string): void {
    let address;
    try { address = ipaddr.parse(value); } catch {
