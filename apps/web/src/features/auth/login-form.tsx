...
export function LoginForm({ onLoggedIn }: LoginFormProps) {
  ...
  async function handleVerify(event: FormEvent) {
    ...
    setBusy(true);
    try {
      const user = await verifyPhoneLogin(phone.trim(), code.trim());
      onLoggedIn(user);
    } catch (err) {
