...
>
> 当前 K3.1 只实现进程内 `pause/resume/cancel` Command、Runtime Lifecycle Snapshot 和 SSE 控制事件，不持久化 Interrupt、Resume Command、Run Version 或幂等键。K3.2 的 clarification、Tool Approval 和 Steer endpoint/schema 仍未冻结；下文 `/clarification + questionId` 与 `/steer` 是长期目标草案，不可直接作为当前 K3 API。
