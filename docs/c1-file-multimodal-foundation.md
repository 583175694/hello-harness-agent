...
> **C1-B 分阶段口径（2026-09-07）**：C1-B 第一阶段先完成 TXT、Markdown、CSV、JSON、PDF 的基础解析和 Web 文件问答闭环。文件上传后立即异步解析；在首版文件处理上限内，Run 创建时注入完整规范化内容。超出文件处理上限的文件明确标记为不可用，不静默截断。第二阶段再基于 Context Engineering 实现大文本分片、检索、摘要和按预算注入。第一阶段仍须保留稳定的解析器版本、内容哈希和基础定位信息，为后续 FilePart 扩展提供兼容底座。
...
| C1-B.2 大文本能力  | P1     | C1-B.1 增强               | FilePart 分片、相关片段选择、摘要/索引、Context Engineering 按预算注入                        |
...
- C1-B.2 再将全文注入替换为 FilePart 选择和 Context Engineering 按预算注入；不改变 `fileId`、附件绑定和文件生命周期协议。
...
- C1-B.2 再增加大文件的片段预览、截断标识和本轮实际使用范围。
...
- **上下文编排**：C1-B.1 只做当前模型可用预算检查和明确超限；C1-B.2 再做 FilePart 选择、裁剪、摘要和动态预算，确保图片和文本不会无界注入模型上下文。
...
- `FilePart`：文本片段、页码/行号/字符区间和截断信息。C1-B.1 先建立基础定位和解析结果边界，C1-B.2 再启用可查询分片；不得因为第一阶段尚未做智能检索而把正文复制到 Message 或 Transcript；
- 解析器版本、文本注入预算和定位投影。

不要为了满足 C1-A0 预先实现通用 `FilePart`、全文解析或复杂来源系统。C1-B.1 可以建立最小的规范化内容和基础定位信息，但不实现面向大文本检索的可查询分片；可查询 `FilePart` 属于 C1-B.2。
...
### 11.4 C1-B.2 大文本能力（后续）

1. 超出 C1-B.1 文件内容上限的文件可以保留原始对象，并通过重新处理进入分片流程。
2. FilePart 具备稳定顺序、定位、contentHash 和截断信息；不同文件的片段不能混淆。
3. Context Engineering 根据用户问题、文件结构和模型预算选择片段；模型看到的内容范围可恢复、可解释。
...
### C1-B.2：大文本与 Context Engineering

1. 增加可查询 `FilePart`、分片摘要或索引，以及行号/页码/字符范围定位。
2. 按问题相关性和模型 Context 预算选择文件片段，支持多文件公平分配和明确截断信息。
3. 评估是否引入 LLM 摘要、关键词索引或向量检索；这些策略不在 C1-B.1 中预先冻结。

### C1-C：后续增强

按实际瓶颈再评估直传/分片上传、异步 Worker 恢复、OCR、复杂来源系统、更多格式和供应商 Files API。

本轮 C1-A1 完成条件是图片链路具备多图、失败恢复、粘贴和清理补偿能力，并通过协议、前端和构建回归；真实 COS 故障注入与完整 E2E 仍作为后续验收项。C1-B.1 的完成条件是五种目标格式完成基础解析、异步状态恢复、限制内全文问答和明确超限边界；C1-B.2 才负责大文本分片和 Context Engineering 注入。所有未支持能力必须显示为明确 unavailable，而不是伪造成功。
