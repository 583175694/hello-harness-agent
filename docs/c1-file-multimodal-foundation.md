...
2. 删除会话后附件关系不可见，COS 清理结果可观测，失败清理可补偿。
3. 类型伪装、损坏图片、超限图片不会进入模型上下文；常规日志不记录图片正文或签名 URL。
...
2. 增加发送图片的尺寸/detail 记录、请求预算保护、集成/E2E 和真实 DeepSeek/GLM 验证。
...
COS_ENDPOINT=https://hello-agent-1256175414.cos.ap-guangzhou.myqcloud.com
```
