...
- C1-A1 增加桌面端将剪贴板图片直接粘贴到 Composer 的能力；粘贴图片与文件选择、拖拽共用同一个前端上传回调和服务端上传接口，不新增独立文件协议。普通短文本粘贴保持原有输入框行为；长文本粘贴外置为 TXT 附件属于 C1-B.3。
- 附件状态：`selected -> uploading -> processing -> ready | failed | rejected`。上传接口在原始文件保存成功后即可返回 `processing`，解析在后台立即开始；`ready` 只表示基础解析完成且文件在当前版本可用，不表示全文已经进入模型上下文。
...
- 文本类：保留 UTF-8 文本，必要时进行受控编码检测；第一阶段不承诺任意历史编码的无损识别。
- CSV/JSON：提供可读文本视图和基本结构元数据，不承诺表格计算或 schema 推断。
- PDF：提取文本、页数和基础页码定位；扫描件 OCR 属于可选增强，未启用时明确失败或 unavailable，不伪造为可读文本。
...
- C1-B.1 对超过文件处理上限的文件显示“当前版本暂不支持”，不显示误导性的“已截断成功”；在限制内的文本/PDF 提供受限预览和基础定位。
- C1-B.2 的工具结果带文件名、`fileId` 和行号/页码范围；工具结果超出限制时必须明确返回错误或不完整标记，不静默截断。
...
- **上下文编排**：C1-B.1 只做当前模型可用预算检查和明确超限；C1-B.2 只做按需文件读取和工具结果预算，确保图片和文本不会无界注入模型上下文。
...
| 单会话文件总量     |                            100 MiB |
| PDF 页数           |                             200 页 |
| 单文件解析内容上限 |    40,000 Unicode code points/文件 |
...
- `FilePart`：文本片段、页码/行号/字符区间和截断信息。C1-B.2 通过文件读取工具返回有限材料；不得把完整正文复制到 Message 或 Transcript；
- 解析器版本、文本注入预算和定位投影。

不要为了满足 C1-A0 预先实现通用 `FilePart`、全文解析或复杂来源系统。C1-B.1 建立规范化内容和基础定位信息；C1-B.2 只增加按需搜索与按行读取，不预先生成分块对象或向量索引。
...
选择/拖拽文件
  ...
  -> 解析/缩略图处理
  -> processing（后台立即解析）
  -> ready（可预览、在首版限制内可作为上下文）
  -> 用户发送消息并绑定 AttachmentRef
  -> 模型按需调用文件工具，Context Compiler 按预算管理工具结果/图片
  -> Agent 回答并返回文件定位信息
```
...
2. 支持桌面端从剪贴板粘贴图片；检测到图片剪贴板数据时阻止默认二进制粘贴并进入与文件选择相同的上传、预览、处理和错误状态流程；普通短文本粘贴不受影响，长文本粘贴按 C1-B.3 外置为 TXT 附件。
3. 删除会话后附件关系不可见，COS 清理结果可观测，失败清理可补偿。
4. 类型伪装、损坏图片、超限图片不会进入模型上下文；常规日志不记录图片正文或签名 URL。

### 11.3 C1-B 文件基础

1. 上传后立即开始异步解析；TXT/Markdown/CSV/JSON/PDF 在基础能力范围内可进入 `ready`，多个附件不会混淆。
2. `ready` 文件在首版解析内容上限内可在同一会话提问；Run 创建时还要检查当前模型和 Session 的实际 Context 预算。
3. PDF 至少支持页码定位，文本支持行号或字符区间定位；保存 parser version 和 contentHash。
4. 超出文件处理上限、当前 Context 预算、解析失败、超时或 OCR 未启用等状态向用户明确呈现，不伪造成功、不静默截断。
...
### 11.5 C1-B.3 长文本粘贴外置为 TXT 附件

> C1-B.3 是现有 C1-B 文件能力的 Composer 输入入口增强，不是独立的文件子系统或新的附件协议。自动生成的 TXT 与用户手动上传的 TXT 在服务端遵循同一套生命周期。

1. Web Composer 读取剪贴板的 `text/plain` 内容；不解析 HTML，不根据内容自动推断 Markdown、JSON、CSV 或代码语言，所有自动生成文件统一使用 UTF-8 `text/plain` 和 `.txt` 扩展名。
2. 粘贴处理顺序固定为：先检查图片文件；存在图片时优先走现有图片粘贴流程；没有图片时再判断纯文本长度。纯文本不超过首版默认阈值 `4,000` 个 Unicode code points 时保持浏览器默认粘贴行为，超过该阈值时阻止默认粘贴并生成 `pasted-text.txt` 附件。该阈值只是可调整的前端交互参数，不属于后端文件解析限制，也不纳入协议兼容承诺。
3. 长文本附件通过现有 `onAttachmentSelected`、文件上传接口和 `FileProcessingService` 处理，不新增 `/paste` 接口、附件类型、数据库字段、文件表或 migration。自动生成的 TXT 与用户选择的 TXT 文件遵循相同的上传、解析、预览、失败、重试、移除和发送流程。
4. 当前输入框已有任务文字必须保留；长文本只作为附件加入，不同时写入 textarea，不自动提交，也不自动生成任务描述。首版仍要求用户输入任务文字后发送，沿用现有 Composer 提交规则。
5. 首版可以统一使用 `pasted-text.txt`。如果同一条消息存在多个自动生成的文本附件，再使用 `pasted-text-2.txt`、`pasted-text-3.txt` 等轻量递增命名；文件名只要求在当前草稿中可读，真正唯一身份由服务端 `fileId` 保证。该命名不进入协议或数据库，也不应引入独立的粘贴草稿状态机。
6. 长文本判断使用 Unicode code points 计数，例如 JavaScript 中使用 `[...text].length`，不得使用会按 UTF-16 code units 计数的 `text.length` 作为产品阈值判断。原始换行、中文、代码、日志和 JSON 文本不得被静默截断或改写。
7. 长文本自动生成的文件仍受服务端文件大小、解析内容和会话附件数量限制。超过 `MAX_PARSED_CODE_POINTS` 时必须进入明确失败或拒绝状态，不得上传前静默截断；附件数量上限沿用当前协议限制，用户文案使用“附件”而不是“图片”。
8. 图片与文本同时存在时图片优先；Composer disabled、等待用户输入或其他不允许附件操作的状态不得触发长文本附件上传。短文本、图片粘贴、拖拽和文件选择的既有行为不得回归。首版不引入 Codex TUI 风格的 `pending_pastes`、占位符展开、提交时物化或粘贴专属持久化。
...
1. 将现有图片专用处理链路扩展为通用文件上传；支持 TXT/Markdown/CSV/JSON/PDF 的 MIME、大小和解析校验。
2. 上传成功后立即异步解析，生成规范化内容、基本结构概览、parser version、contentHash 和基础定位信息。
3. 在首版文件限制内支持全文注入；创建 Run 时动态检查模型 Context 预算，超限明确阻止并说明原因。
4. Web 提供 processing/ready/failed/rejected 状态、受限预览和失败重试；不实现智能分片检索。
...
### C1-B.3：长文本粘贴外置为 TXT 附件

1. 在 Web Composer 中增加纯文本粘贴分流：图片剪贴板内容优先；不超过 `4,000` 个 Unicode code points 的文本继续使用浏览器默认粘贴；超过阈值的文本阻止默认粘贴并生成 `text/plain` 的 `pasted-text.txt` 附件。
2. 自动生成的文件通过现有 `onAttachmentSelected` 和文件上传链路处理，复用 `processing -> ready | failed | rejected` 状态、文本预览、移除、重试、发送和按需读取能力。
3. 保留当前输入框已有任务文字，不把长文本重复写入 textarea，不自动提交，不新增数据库表、字段、接口、附件类型或 migration。
4. 首版测试聚焦 Composer 分流和内容保真：短文本默认粘贴、超过阈值后阻止默认行为、生成文件的 MIME/名称/内容、Unicode code points 计数、图片优先和既有行为不回归。TXT 上传、解析、预览、失败和按需读取继续复用并依赖已有文件链路测试；不重复建立一套粘贴专属生命周期测试。

### C1-C：后续增强

按实际瓶颈再评估直传/分片上传、异步 Worker 恢复、OCR、复杂来源系统、更多格式和供应商 Files API。

本轮 C1-A1 完成条件是图片链路具备多图、失败恢复、粘贴和清理补偿能力，并通过协议、前端和构建回归；真实 COS 故障注入与完整 E2E 仍作为后续验收项。C1-B.1 的完成条件是五种目标格式完成基础解析、异步状态恢复、限制内全文问答和明确超限边界；C1-B.2 负责 COS 规范化正文、按需文件工具和 Context 结果预算，不实现预分块；C1-B.3 作为输入入口增强，完成长文本按可调前端阈值稳定转为 TXT 附件，并复用现有文件生命周期和预览链路，不新增独立状态、协议或持久化模型。所有未支持能力必须显示为明确 unavailable，而不是伪造成功。
