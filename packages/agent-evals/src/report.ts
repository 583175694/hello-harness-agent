...
function markdown(report: EvalRunReport): string {
  ...
  for (const item of report.cases) {
    lines.push(
      `| ${item.caseId} | ${item.hardPassed ? 'PASS' : 'FAIL'} | ${item.judge?.overallScore ?? 'N/A'} | ${item.metrics.searchCalls} | ${item.metrics.fetchCalls} | ${item.metrics.uniqueDocuments} | ${item.metrics.passageCharacters} | ${item.durationMs}ms | ${escapeMarkdown(item.error ?? item.judgeError ?? item.cleanupError ?? '')} |`,
    );
  }
...
function detailedReview(report: EvalRunReport): string {
  ...
  for (const item of report.cases) {
    lines.push(
      ...
      '',
      `- 类别：${item.category}`,
      `- 硬规则：${item.hardPassed ? '通过' : '失败'}`,
      `- 耗时：${item.durationMs}ms`,
      `- 模型：${item.model ?? '未知'}`,
      `- Provider：${item.provider ?? '无搜索 Provider'}`,
      '',
    );
    lines.push(
      '### 问题',
      ...
      '### Agent 最终结果',
      '',
      item.answer || '没有持久化 assistant 结果。',
      '',
    );
    lines.push('### 自动诊断', '');
    ...
    lines.push(
      ...(failed.length
        ? failed.map((rule) => `- [失败] ${rule.id}：${rule.detail}`)
        : ['- 没有硬规则失败。']),
      '',
    );
    lines.push(
      '### 执行指标',
      '',
      `- Search 调用：${item.metrics.searchCalls}`,
      `- Fetch 调用：${item.metrics.fetchCalls}`,
    ...
    lines.push(
      ...(item.executions.length
        ? item.executions.map(
            (execution) =>
              `- ${execution.toolName} / ${execution.status} / ${execution.durationMs}ms / call=${execution.toolCallId}${execution.error ? ` / ${execution.error.code}: ${execution.error.detail}` : ''}`,
          )
        : ['- 没有持久化工具执行。']),
      '',
    );
      ...
      ...(item.sources.length
        ? item.sources.map((source) =>
            source.kind === 'fetched'
              ...
              : `- clue ${source.title}：${source.url}，provider=${source.provider}，used=${source.used}`,
          )
        : ['- 没有持久化来源。']),
      '',
    );
    ...
    if (item.judge) {
      ...
      lines.push(
        `- 任务完成度：${item.judge.taskCompletion.score}/5，${item.judge.taskCompletion.reason}`,
      );
      lines.push(
        `- 来源质量：${item.judge.sourceQuality.score}/5，${item.judge.sourceQuality.reason}`,
      );
      lines.push(
        `- 事实支撑：${item.judge.groundedness.score}/5，${item.judge.groundedness.reason}`,
      );
...
function humanReviewCsv(results: EvalCaseResult[]): string {
  ...
  for (const item of [...results].sort(
    (left, right) => stableNumber(left.caseId) - stableNumber(right.caseId),
  )) {
  ...
  const rows = [...selected.values()].map((item) => [
    item.caseId,
    item.category,
    String(item.hardPassed),
    String(item.judge?.overallScore ?? ''),
    item.judge?.verdict ?? '',
    item.prompt,
    item.answer ?? '',
    item.hardRules
